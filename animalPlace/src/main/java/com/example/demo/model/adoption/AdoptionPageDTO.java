...
@Data
public class AdoptionPageDTO {
	private int startPage;
	private int endPage;
	private int realEnd;
	private long total;
	private boolean prev, next;
	private AdoptionCriteria adCri;
	
	public AdoptionPageDTO(long total,AdoptionCriteria adCri) {
		this.adCri = adCri;
		this.total = total;
		
		this.endPage = (int)Math.ceil(adCri.getPageNum()/(double)adCri.getPageSize())*adCri.getPageSize();
		this.startPage = this.endPage - (adCri.getPageSize()-1);
		this.realEnd = (int)Math.ceil(total/(double)adCri.getPageSize());
